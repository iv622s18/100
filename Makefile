...
SOURCES_TEST = matches_test.c main.c validation_test.c
DIT = build/test
DAT = test
DUT = bin/test
...
$(DIT)/matches_test.o: $(DAT)/matches_test.c
	@if [ ! -d $(DIT) ] ; then echo "creating $(DIT)" ; mkdir -p build; mkdir build/test; fi
	$(CC) $(CFLAGS) -c $(DAT)/matches_test.c -o $(DIT)/matches_test.o 

$(DIT)/validation_test.o: $(DAT)/validation_test.c
	@if [ ! -d $(DIT) ] ; then echo "creating $(DIT)" ; mkdir -p build; mkdir build/test; fi
	$(CC) $(CFLAGS) -c $(DAT)/validation_test.c -o $(DIT)/validation_test.o

$(DUT)/$(EXECUTABLE_TEST): $(DIT)/main.o $(DIT)/matches_test.o $(DIT)/validation_test.o $(DIR)/func.o
	@if [ ! -d $(DUT) ] ; then echo "creating $(DUT)" ; mkdir -p bin; mkdir -p bin/test; fi
	$(CC) $(DIT)/main.o $(DIT)/matches_test.o $(DIT)/validation_test.o $(DIR)/func.o -o $(DUT)/$(EXECUTABLE_TEST) -lm
